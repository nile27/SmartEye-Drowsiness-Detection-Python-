...
### 1. 가상환경(.venv) 도입 계기
*   **크로스 플랫폼 대응**: 개발은 맥북(macOS)에서 진행했으나 채점 및 구동 환경은 윈도우 등 타 OS 환경일 수 있어 라이브러리 및 실행 불일치 문제를 예방하고자 적용했습니다.
*   **환경 민감성 극복**: 이전 프로젝트 수업 시 폴더 경로 및 환경 문제로 실행 에러를 겪었던 기억이 있어, 도커(Docker)처럼 운영체제 환경에 크게 구애받지 않고 가볍게 격리되어 실행되는 방식을 찾다가 `.venv`를 선택했습니다.

### 2. 딥러닝 모델 학습 실패와 극복
*   **초기 시도**: 직접 수집한 이미지 데이터셋으로 캐글(Kaggle)에서 딥러닝 모델 학습 시도.
*   **실패 원인**: 눈 감김 상태에 관한 학습용 **데이터셋의 절대적 부족**으로 학습 및 탐지 정확도 저하.
*   **해결 방안**: 얼굴 특징 좌표를 정밀하게 알려주는 MediaPipe를 기반으로, 가로세로 비율(EAR) 계산 수식으로 선회.
*   **최종 결과**: 데이터셋 학습 과정 없이 웹캠만 있으면 어느 환경에서나 즉각 작동하는 실시간 졸음 감지 구현.
